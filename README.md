...
3. Создайте `.env` (по примеру `.env.example` в корне или `backend/.env`):
   ...
   ```
   Для фронтенда скопируйте `frontend/.env.example` → `frontend/.env` (или задайте свой `VITE_API_URL`).

4. Примените миграции и заполните демо-данные (Prisma автоматически сгенерирует клиент через `npm run prepare -w backend`):
   ```bash
   cd backend
